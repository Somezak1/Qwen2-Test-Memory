...
class GenerationMixin:
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        # logits_processor: [<transformers.generation.logits_process.RepetitionPenaltyLogitsProcessor object at 0x7f6c603c4130>]
        # stopping_criteria: [<transformers.generation.stopping_criteria.MaxLengthCriteria object at 0x7f6c603c4190>, <transformers.generation.stopping_criteria.EosTokenCriteria object at 0x7f6c603c4250>]
        # generation_config: <GenerationConfig instance at 0x7f6c603c4280>
        # synced_gpus: False
        # streamer: None
        # logits_warper: [<transformers.generation.logits_process.TemperatureLogitsWarper object at 0x7f6c603c41c0>, <transformers.generation.logits_process.TopKLogitsWarper object at 0x7f6c603c44c0>, <transformers.generation.logits_process.TopPLogitsWarper object at 0x7f6c603c4520>]
        ...
        # pad_token_id: tensor(151643, device='cuda:0')
        # output_attentions: False
        # output_hidden_states: False
        # output_scores: False
        # output_logits: None
        ...
        def record(s, time=''):
            ...
            print("\n\n" + "=" * 90 + s.center(80) + "=" * 90)
            print(print1 + print2 + print3 + time)


        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):

            record(f"Before Step {cnt} Before prepare inputs in utils.py GenerationMixin._sample()")  # 【4】

            # prepare model inputs
            model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
            # 第一次循环, 即为预填充准备数据时:
            # model_kwargs: {
            #     'cache_position': [bsz, s], torch.int64, cache_position[i] = range(s)
            #     'use_cache': True
            ...
            #     'past_key_values': None
            #     'use_cache': 同model_kwargs["use_cache"]
            #     'attention_mask': 同model_kwargs["attention_mask"]
            # }

            # 后续循环, 即为第 j 次解码准备数据时 (j 从 1 开始):
            # model_kwargs: {
            #     'cache_position': [bsz, 1], torch.int64, cache_position[i] = s + j - 1
            ...
            #     'past_key_values': 同model_kwargs["past_key_values"]
            #     'use_cache': 同model_kwargs["use_cache"]
            #     'attention_mask': 同model_kwargs["attention_mask"]
            # }

            record(f"Before Step {cnt} After  prepare inputs in utils.py GenerationMixin._sample()")  # 【5】
            ...
            # 需要特别关注 outputs.past_key_values 和 outputs.logits
            #
            # outputs.past_key_values:
            #     model_inputs 和 model_kwargs 也指向了 outputs.past_key_values
            #     因此就算在第 j 轮循环中已经计算出了新的 outputs
            #     但第 j-1 轮循环中的 outputs.past_key_values 依然被 model_inputs 和 model_kwargs 指着, 这块空间并未得到释放
            #     只有在第 j+1 轮循环执行完 model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs) 后
            #     第 j-1 轮的 outputs.past_key_values 才会完全得到释放
            #     此时 model_inputs 和 model_kwargs 指向 第 j 轮的 outputs.past_key_values
            #
            # outputs.logits:
            #     在第 j 轮循环刚计算出新的 outputs 时, next_token_logits 指向的依然是上一轮循环的 outputs.logits
            #     只有在第 j 轮循环执行完 next_token_logits = outputs.logits[:, -1, :] 后
            #     next_token_logits 才会指向这一轮循环的 outputs.logits

            e = time.time()
            record(f"After Step {cnt} in utils.py GenerationMixin._sample()", time=f"[Time]: {e-s:.3f} s")  # 【23】
            s = e

            # 第一次循环, 即推理预填充时:
            #     【23】 比 【5】 在 memory allocated 部分多了:
            #         outputs.logits: [bsz, s, vocab_size], torch.float32
            #         outputs.past_key_values: ( (tensor, tensor)_1, (tensor, tensor)_2, ..., (tensor, tensor)_num_hidden_layers ), tensor.size = [bsz, num_key_value_heads, s, head_dim], torch.bfloat16
            #         8.125MB: 这一项是 Pytorch 第一次前向传播时在显存中固定占用的数值, 该占用有且仅有一个
            #
            #     outputs: [logits, past_key_values, hidden_states=None, attentions=None]
            ...
            # 后续第 j+1 次循环, 即推理第 j 次解码时 (j 从 1 开始):
            #     【23】 比 【5】 在 memory allocated 部分多了:
            #         outputs.logits: [bsz, 1, vocab_size], torch.float32
            #         outputs.past_key_values: ( (tensor, tensor)_1, (tensor, tensor)_2, ..., (tensor, tensor)_num_hidden_layers ), tensor.size = [bsz, num_key_value_heads, s+j, head_dim], torch.bfloat16
            #
            #     outputs: [logits, past_key_values, hidden_states=None, attentions=None]
            ...
            # [:, -1, :] 操作不开辟新的空间进行存储, 只要 next_token_logits 不改变指向或不删除, outputs.logits 所占用的空间就不会得到释放
            # next_token_logits: [bsz, vocab_size], torch.float32

            # pre-process distribution
            next_token_scores = logits_processor(input_ids, next_token_logits)
            # next_token_scores: [bsz, vocab_size], torch.float32

            record(f"After Step {cnt} Before logits_warper in utils.py GenerationMixin._sample()")  # 【24】

            # 在第一次循环时, 【24】 相对 【23】 在 memory allocated 部分增加了 next_token_scores 张量
            # 在第二次循环时, 【24】 相对 【23】 在 memory allocated 部分少了 logits: [bsz, s, vocab_size], torch.float32
            #               在执行 next_token_logits = outputs.logits[:, -1, :] 之前, 虽然已是第二次循环了, 但第一次循环的 outputs.logits 仍由第一次循环的 next_token_logits 指着
            #               在执行 next_token_logits = outputs.logits[:, -1, :] 之后, outputs.logits 指向了第二次循环的 outputs.logits, 第一次循环的 outputs.logits 得以释放
            # 在后续循环时,   【24】 相对 【23】 在 memory allocated 部分少了 logits: [bsz, 1, vocab_size], torch.float32
            #               在执行 next_token_logits = outputs.logits[:, -1, :] 之前, 上一次循环的 outputs.logits 仍由 next_token_logits 指着
            #               在执行 next_token_logits = outputs.logits[:, -1, :] 之后, outputs.logits 指向了这次循环的 outputs.logits, 上一次的 outputs.logits 得以释放

            # 在第一次循环时, 【24】 相对 【4】 在 memory allocated 部分增加了
            # 	            model_inputs.position_ids, [bsz, s], torch.int64
            # 	            outputs.logits: [bsz, s, vocab_size], torch.float32
            # 	            outputs.past_key_values: ( (tensor, tensor)_1, (tensor, tensor)_2, ..., (tensor, tensor)_num_hidden_layers ), tensor.size = [bsz, num_key_value_heads, s, head_dim], torch.bfloat16
            # 	            8.125MB: 这一项是 Pytorch 第一次前向传播时在显存中固定占用的数值, 该占用有且仅有一个
            # 	            next_token_scores, [bsz, vocab_size], torch.float32
            # 在第二次循环时, 当前循环的 【24】 相对第一次循环的 【24】 在 memory allocated 部分增加了
            #               probs: [bsz, vocab_size], torch.float32
            #               next_tokens: [bsz, ], torch.int64
            #               input_ids: [bsz, pre_len+1]
            #               model_kwargs.attention_mask: [bsz, s+1], torch.int64
            #               model_inputs.position_ids: [bsz, 1], torch.int64
            #               - outputs.logits: [bsz, s, vocab_size], torch.float32（- 代表这一项是减少）
            #               outputs.logits: [bsz, 1, vocab_size], torch.float32
            #               outputs.past_key_values: ( (tensor, tensor)_1, (tensor, tensor)_2, ..., (tensor, tensor)_num_hidden_layers ), tensor.size = [bsz, num_key_value_heads, s+j, head_dim], torch.bfloat16
            # 后续循环时 (第j+1次循环, j≥2) , 当前循环的 【24】 相对上一次循环的 【24】 在 memory allocated 部分
            #               model_kwargs.past_key_values 中的 tensor.size 从 [bsz, num_key_value_heads, s+j-2, head_dim] 变为 [bsz, num_key_value_heads, s+j-1, head_dim]
            #               outputs.past_key_values      中的 tensor.size 从 [bsz, num_key_value_heads, s+j-1, head_dim] 变为 [bsz, num_key_value_heads, s+j, head_dim]
            ...
            record(f"After Step {cnt} After  logits_warper in utils.py GenerationMixin._sample()")  # 【25】
            # 【25】 相对 【24】 memory allocated 部分没变化
            ...
            #     logits: [bsz, s, vocab_size], torch.float32
            #     past_key_values: ( (tensor, tensor)_1, (tensor, tensor)_2, ..., (tensor, tensor)_num_hidden_layers ), tensor.size = [bsz, num_key_value_heads, s, head_dim], torch.bfloat16
            # 第一次循环, 即预填充后, self._update_model_kwargs_for_generation(...) 变换前:
            # model_kwargs: {
            #     'use_cache': True
            #     'attention_mask': [bsz, s], 全 1 张量, torch.int64
            #     'cache_position': [bsz, s], torch.int64, cache_position[i] = range(s)
            # }
            # 第一次循环, 即预填充后, self._update_model_kwargs_for_generation(...) 变换后:
            # model_kwargs: {
            #     'use_cache': True,
            #     'attention_mask': [bsz, s+1], 全 1 张量, torch.int64
            #     'cache_position': [bsz, 1], torch.int64, cache_position[i] = s + j - 1
            #     'past_key_values': ( (tensor, tensor)_1, (tensor, tensor)_2, ..., (tensor, tensor)_num_hidden_layers ), tensor.size = [bsz, num_key_value_heads, s, head_dim], torch.bfloat16
            # }
            ...
            #     logits: [bsz, 1, vocab_size], torch.float32
            #     past_key_values: ( (tensor, tensor)_1, (tensor, tensor)_2, ..., (tensor, tensor)_num_hidden_layers ), tensor.size = [bsz, num_key_value_heads, s+j, head_dim], torch.bfloat16
            # 第 j + 1 次循环, 即第 j 次解码后 (j 从 1 开始), self._update_model_kwargs_for_generation(...) 变换前:
            # model_kwargs: {
            #     'use_cache': True,
            #     'attention_mask': [bsz, s+j], 全 1 张量, torch.int64
            #     'cache_position': [bsz, 1], torch.int64, cache_position[i] = s + j - 1
            #     'past_key_values': ( (tensor, tensor)_1, (tensor, tensor)_2, ..., (tensor, tensor)_num_hidden_layers ), tensor.size = [bsz, num_key_value_heads, s+j-1, head_dim], torch.bfloat16
            # }
            # 第 j + 1 次循环, 即第 j 次解码后 (j 从 1 开始), self._update_model_kwargs_for_generation(...) 变换后:
            # model_kwargs: {
            #     'use_cache': True,
            #     'attention_mask': [bsz, s+j+1], 全 1 张量, torch.int64
            #     'cache_position': [bsz, 1], torch.int64, cache_position[i] = s + j - 1 + 1
            #     'past_key_values': ( (tensor, tensor)_1, (tensor, tensor)_2, ..., (tensor, tensor)_num_hidden_layers ), tensor.size = [bsz, num_key_value_heads, s+j , head_dim], torch.bfloat16
            # }
